/**
 * Simulation: recentOpenQuestion zählt eine Frage nur dann als "offen",
 * wenn sie jünger als Y Stunden ist.
 *
 * Bewusst NICHT mit der Pausenregel kombiniert — nur dieser eine Hebel wird
 * verändert, damit seine Wirkung sauber zuzuordnen bleibt.
 *
 * segmentation-v4.mjs bleibt unangetastet: je Y wird eine Kopie mit genau
 * dieser einen Ersetzung nach /tmp geschrieben und dynamisch geladen.
 *
 * Gemessen wird gegen die GEKLÄRTE gemeinsame Fassung. Nur Zahlen.
 *
 * Aufruf: node scripts/experiment-fragenalter.mjs <rounds> <chunks> <marks> <submissions> <resolutions>
 */
...
const ANCHOR_QUESTION = "    if (hasQuestion(text)) return true;";
...
if (source.split(ANCHOR_QUESTION).length - 1 !== 1) throw new Error('Anker nicht eindeutig');

async function loadVariant(hours) {
  // Eine Frage hält die Konversation nur zusammen, solange sie frisch ist.
  const replacement = `    if (hasQuestion(text)) {\n`
    + `      const ageMinutes = (timestamp(messages[currentIndex]) - timestamp(messages[index])) / 60;\n`
    + `      return Number.isFinite(ageMinutes) && ageMinutes < ${hours * 60};\n`
    + `    }`;
  const variant = source.replace(ANCHOR_QUESTION, replacement);
  if (variant === source) throw new Error('Ersetzung hat nichts verändert');
  const path = `/tmp/segmentation-frage-${hours}.mjs`;
...
for (const [round, missed] of baseline.missedReasons) {
  const set = new Set();
  for (const [position, reason] of missed) if (reason === 'open_question_continuation') set.add(position);
...
console.log(`davon übersehen mit Grund open_question_continuation: ${pauseTargetCount}\n`);

console.log('   Y | Grenzen | getroffen | Fehlalarme | übersehen |     F1 | davon der Fragen-Fälle');
console.log('-----+---------+-----------+------------+-----------+--------+-----------------------');
console.log(
...
for (const hours of HOURS) {
  const segment = await loadVariant(hours);
