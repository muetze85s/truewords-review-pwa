...
async function loadVariant(kind, hours) {
  ...
  const path = `/tmp/segmentation-${kind}-${String(hours).replace('.', '_')}.mjs`;
...
console.log('=== Einbau unbedingt (Feinlauf am linken Rand) ===');
...
for (const hours of HOURS) {
  const segment = await loadVariant('unbedingt', hours);
  ...
  if (!best || result.f1 > best.f1) best = { hours, f1: result.f1 };
  console.log(
...
console.log(`\nbester getesteter Wert in diesem Lauf: ${best.hours}h mit F1 ${best.f1.toFixed(4)}`);
